...
echo "$($PSQL "SELECT name FROM games AS g
JOIN teams AS t ON g.winner_id = t.team_id OR g.opponent_id = t.team_id 
...
echo "$($PSQL "SELECT DISTINCT year, name FROM games INNER JOIN teams ON games.winner_id = teams.team_id WHERE round='Final' ORDER BY year;")"
